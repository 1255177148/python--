...
class TencentRecruit:
    ...
    recruitUrl = 'https://careers.tencent.com/tencentcareer/api/post/Query' # 招聘信息分页接口
    headers = {
    ...
    def getSelectData(self):
        ...
        获取腾讯招聘左侧筛选框的数据
        '''
        res = requests.get(url=self.selectListUrl,params=self.params,headers=self.headers)
    ...
    def getPageData(self, params):
        ...
        pageTotalNum = math.ceil(pageCount / pageSize) # 使用math.ceil向上取整，即如果有小数，则整数部分+1，然后只取最后的整数部分
...
if __name__ == '__main__':
    ...
    for item in oneSelectParam:
        if item['parentId'] and ('40003' == item['parentId'] or '40004' == item['code']):
